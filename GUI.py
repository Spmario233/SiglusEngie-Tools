...
tool=["解包Scene","打包Scene","解密Gameexe","加密Gameexe",
      "解包ss","打包ss","解包dbs","打包dbs","解包pck","打包pck",
      "裁剪OMV文件头"]
...
def running(cmd,key):
    ...
    if check:
        ...
        messagebox.showwarning("警告","输入参数有误！")
    cmdText['state']='disabled'
...
class setScenePacker:
    def __init__(self):
        ...
        name3=Label(inputFrame,text="输出文件夹:")
        name1.grid(row=0,padx=2,sticky='w')
        name2.grid(row=2,padx=2,sticky='w')
        ...
        button1=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFile(value1))
        button2=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFolder(value2))
        button3=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:setFile(value3))
        button1.grid(row=1,column=1,padx=2)
        button2.grid(row=3,column=1,padx=2)
        button3.grid(row=5,column=1,padx=2)
        valueC.set('17')
        nameC=Label(inputFrame,text="压缩等级（2-17, 若填0则使用伪压缩）: ")
...
class setGameexeUnpacker:
    def __init__(self):
        ...
        entry2.grid(row=3,column=0,padx=2)
        button1=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFile(value1))
        button2=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:setFile(value2))
        button1.grid(row=1,column=1,padx=2)
        button2.grid(row=3,column=1,padx=2)
    def run(self):
...
class setGameexePacker:
    def __init__(self):
        ...
        entryC.grid(row=7,column=1,padx=2,pady=4)
        valueB.set(False)
        buttonB=Checkbutton(inputFrame,text="二次加密（没啥卵用）",variable=valueB)
...
class setssPacker:
    def __init__(self):
        ...
        button1=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFolder(value1))
        button2=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFolder(value2))
        button3=Button(inputFrame,text="选择",width=BUTTON_WIDTH,command=lambda:selectFolder(value3))
        button1.grid(row=1,column=1,padx=2)
        button2.grid(row=3,column=1,padx=2)
        button3.grid(row=5,column=1,padx=2)
        valueB.set(True)
        buttonB=Checkbutton(inputFrame,text="检测xlsx文件",variable=valueB)
        buttonB.grid(row=8,padx=2,pady=4,sticky='e')
    def run(self):
...
class setdbsDecrypt:
    def __init__(self):
        ...
        button1.grid(row=1,column=1,padx=2)
        valueB.set(False)
        buttonB=Checkbutton(inputFrame,text="不过滤纯英文文本",variable=valueB)
        buttonB.grid(row=8,padx=2,pady=4,sticky='e')
    def run(self):
...
root=Tk()
root.title("Siglus Tools 吃饱了撑的汉化版")
...
startButton=Button(keyFrame,text="开始",command=start,width=7)
...
try:
    ...
    hasSkf=True
    kfButton=Button(keyInfo,text="搜索密钥",command=clickFindKey,width=8)
    kfButton.pack(side='right',padx=PAD,pady=PAD,anchor='e')

keyLabel=Label(keyInfo,text='解密密钥（使用 "," 进行分隔）:')
...
optionLabel=Label(optionFrame,text="选项:")
